...
class LeadCompat {
    ...
    public function apiList($parameters = []) {
        ...
        // FIX[iter7b]: проброс ?with=... (v4). Нужно для loss_reason — без него
        // amoCRM не отдаёт причину отказа. Принимаем строку 'loss_reason' или
        // массив ['loss_reason','contacts',...].
        if(!empty($parameters['with'])) {
            $query['with'] = is_array($parameters['with'])
                ? implode(',', $parameters['with'])
                : $parameters['with'];
        }
    ...
    public function apiGetByIdV4($lead_id, $with = []){
        $query = [];
    ...
    public function apiGetNotesV4($lead_id, $query = []){
        return Http::request(
            $this->baseDomain,
            $this->accessToken,
            'GET',
            '/api/v4/leads/' . (int)$lead_id . '/notes',
            $query
        );
    }

    public function apiGetEventsV4($lead_id, $page = 1, $limit = 50){
        $query = [
            'page' => (int)$page,
            'limit' => (int)$limit,
            'filter[entity]' => 'lead',
