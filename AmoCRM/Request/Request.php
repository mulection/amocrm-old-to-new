...
namespace Mcrm\Models\Api\AmoCRM\Request;

use DateTime;
use Mcrm\Models\Api\AmoCRM\Exception;
use Mcrm\Models\Api\AmoCRM\NetworkException;
 ...
 *
 * @package Mcrm\Models\Api\AmoCRM\Request
...
class Request
{
    ...
    protected function getV4Request($url, $parameters = []){
        return $this->v4Request('GET', $url, $parameters);
    }

    protected function postV4Request($url, $parameters = []){
        return $this->v4Request('POST', $url, $parameters);
    }

    protected function patchV4Request($url, $parameters = []){
        return $this->v4Request('PATCH', $url, $parameters);
    }

    protected function v4Request($method, $url, $parameters = []){
        $base = 'https://' . $this->parameters->getAuth('domain');
        $endpoint = $base . $url;
        ...
        if(in_array($method, ['POST', 'PATCH']) && !empty($parameters)){
        ...
        return $this->parseV4Response($result, $info);
    }

    protected function parseV4Response($response, $info){
        $result = json_decode($response, true);

        if(floor($info['http_code'] / 100) >= 3){
            if(isset($result['title']) || isset($result['detail'])){
                throw new \Exception(($result['title'] ?? 'API error') . ': ' . ($result['detail'] ?? ''));
            }
